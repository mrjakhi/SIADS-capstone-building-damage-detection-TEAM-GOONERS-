...
def main(cfg: DictConfig):
    ...
    # Scheduler
    scheduler = ReduceLROnPlateau(optimizer, mode=cfg.params.schedulermode, patience=cfg.params.patience, verbose=True)
    ...
    logger_val.close()    
    # Save best model:
    torch.save(best_model.state_dict(), cfg.model.best_mdl)
    # add code here
    return 
...
def load_dataset():
    ...
    # Core function:
    train_images_dir = cfg.paths.sampledata + 'train/images'
    train_incidents = list(set(['_'.join(i.split('_')[:2]) for i in os.listdir(images_dir)])) # Unique incidents
    
    val_images_dir = cfg.paths.sampledata + 'val/images'
    val_incidents = list(set(['_'.join(i.split('_')[:2]) for i in os.listdir(images_dir)])) # Unique incidents 
     
    xBD_train = DisasterDataset(train_images_dir, train_incidents, transform=True, normalize=True)
    xBD_val = DisasterDataset(val_images_dir, val_incidents, transform=True, normalize=True)
