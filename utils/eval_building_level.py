# Result evaluation file taken from https://github.com/microsoft/building-damage-assessment-cnn-siamese
...
def get_feature_info(feature):
    ...
    """
    props = {}
    with open('./utils/xBD_label_map.json') as label_map_file:
        label_name_num = json.load(label_map_file)['label_name_to_num']
    ...
    for feat in feature['features']['xy']:
        ...
        damage_class_num = label_name_num[damage_class]  # get the numerical label
...
def _evaluate_tile(pred_polygons_and_class: list, 
    ...
    for i_pred, (pred_poly, pred_class) in enumerate(pred_polygons_and_class):
        ...
        for i_label, (label_poly, label_class) in enumerate(label_polygons_and_class):
            ...
            if pred_poly.intersects(label_poly):
